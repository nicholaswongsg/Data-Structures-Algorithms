...
void calDegreeV(Graph g, int *degreeV)
{
    ...
    // Write a function calDegreeV() to compute vertex degrees using adjacent lists.
    // Please reuse the work done in Q2.

    int i,j;
    ListNode *temp = NULL;

	for(i=0;i<g.V;i++){ // i less than g.V vertax
		degreeV[i]=0; // Make vertax of index to 0
		ListNode *temp = g.adj.list[i]; // Go into linklist of the vertax
		while(temp != NULL){ // Check if finish
			degreeV[i]++; // Keep count
